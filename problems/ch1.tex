...
There are many problems in this chapter in which we will use this specific Lemma.
So we state it here for convenience.

\begin{lemNotes}[Unique prime factorization/Fundamental Theorem of Algebra]
...
\begin{pr}\leavevmode
    ...
    \begin{enumerate}[label=\textbf{(\alph*)}]
        ...
        \begin{IEEEeqnarray*}{rCl}
            a_0 + a_1\left(\frac{r}{s}\right) + a_2\left(\frac{r}{s}\right)^2 + \ldots
            + a_{m - 1}\left(\frac{r}{s}\right)^{m - 1} + \left(\frac{r}{s}\right)^m & = & 0 \\
